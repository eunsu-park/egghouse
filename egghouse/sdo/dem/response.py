...
def get_temperature_response(
    ...
    Three sources are supported, in this priority order:

    1. ``ssw_table_path`` — an SSW-derived ``.npz`` table generated by
       ``aia_get_response.pro`` (canonical IDL reference; CHIANTI-based).
       This is the preferred path for research-quality work because it
       reuses the calibration used by demregpy and the official AIA pipeline.
    2. ``aiapy`` — currently disabled. aiapy 0.12 removed
       ``Channel.temperature_response``; callers must supply an
       ``ssw_table_path`` until a CHIANTI-backed replacement is wired up
       (e.g. via ``fiasco``). See `_get_aiapy_response` for details.
    3. The built-in Gaussian fallback (``_get_fallback_response``), used
